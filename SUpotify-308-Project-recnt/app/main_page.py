...
from . import db 
from .models import Album, Friendship, RateSong, SongPlaylist, Playlist, Artist, Song, User, RateArtist, RateAlbum, ArtistsOfSong, GenresofArtist
from datetime import datetime
...
def fetch_and_save_song(sp, song_id):
    ...
    for singer in all_artists_list:
        ...
        if not Artist.query.filter_by(artist_id=singer['id']).first():
            ...
            genres_of_artist = artist_info['genres']
            for genre in genres_of_artist:
                print(genre)
                new_genre = GenresofArtist(
                    artist_id = artist_id,
                    genre = genre
                )
