...
const DatabaseMiddle = ({setCurrentBottomSong}) => {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8008/all_songs');
        if (!response.ok) {
  ...
  return (
    <div className="database-middle">
        <h1>Database</h1>
        <PlaylistContainer className= "playlist-container" songs={songs} setCurrentBottomSong={setCurrentBottomSong} />
    </div>
  );
};
