...
def sign_up():
    if request.method == "POST":
        ...
        if existing_user:
            # User with the same user_id already exists, handle accordingly (e.g., return an error response)
            response = {'message': 'User with the same user_id already exists'}
        ...
        if added_user:
            ...
            return jsonify({"message": True, "user_info": user_info})
        else:
            return jsonify({"message": True, "user_info": None})
